...
@section('content')
<div class="mb-5">
    <a href="{{ route('manager.products.index') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-[13px] admin-btn-secondary no-underline border-none cursor-pointer transition-all duration-150">
...
<div class="flex items-center justify-between flex-wrap gap-4 mb-7 max-md:flex-col max-md:items-start">
    <h1 class="text-2xl font-bold admin-text">Edit Produk</h1>
</div>
...
<form method="POST" action="{{ route('manager.products.update', $product) }}" enctype="multipart/form-data" id="productForm">
    @csrf
    @method('PUT')
    ...
    <div class="admin-card border rounded-xl overflow-hidden mb-6">
        ...
        <div class="p-5 space-y-5">
            <div>
                ...
                <input type="text" class="w-full px-3.5 py-2.5 rounded-lg admin-input font-[inherit] text-sm outline-none transition-colors duration-200 focus:border-blue-500" id="name" name="name" value="{{ old('name', $product->name) }}" required placeholder="Nama produk...">
    ...
    <div class="admin-card border rounded-xl overflow-hidden mb-6">
        ...
        <div class="p-5 space-y-5">
            ...
            <div>
                <label class="block text-[13px] font-semibold admin-text-secondary mb-1.5">Galeri Saat Ini <span class="text-xs font-normal admin-text-muted">(centang X untuk menghapus)</span></label>
                <div class="flex flex-wrap gap-3">
                    ...
                    <div class="relative existing-gallery-item" data-id="{{ $img->id }}">
                        ...
                        <button type="button" class="gallery-delete-toggle"
                            ...
                            title="Tandai untuk dihapus" onclick="event.stopPropagation(); toggleDeleteImage(this, {{ $img->id }});">
            ...
            <div>
                <label for="gallery_images" class="block text-[13px] font-semibold admin-text-secondary mb-1.5">
                    Tambah Galeri Foto <span class="text-xs font-normal admin-text-muted">(opsional, bisa pilih beberapa sekaligus)</span>
                </label>
    ...
    <div class="flex gap-2.5 pt-5 pb-2">
        ...
        <a href="{{ route('manager.products.index') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-[13px] admin-btn-secondary no-underline border-none cursor-pointer transition-all duration-150">Batal</a>
    </div>
</form>
...
$(function () {
    ...
    // ══════════════════════════════════════════════════════════════
    window.toggleDeleteImage = function (btn, imgId) {
        var $item = $(btn).closest('.existing-gallery-item');
        var $cb = $item.find('.delete-checkbox');
        var $thumb = $item.find('.gallery-thumb');
        var marked = !$cb.prop('checked');
        $cb.prop('checked', marked);
        $thumb.css('opacity', marked ? '0.3' : '1');
        $(btn).css('background', marked ? '#22c55e' : '#ef4444');
        $(btn).find('.material-symbols-outlined').text(marked ? 'undo' : 'close');
        $(btn).attr('title', marked ? 'Batalkan hapus' : 'Tandai untuk dihapus');
    };
    ...
    // ══════════════════════════════════════════════════════════════
    // Load existing data OR old() data (after validation failure)
    // ══════════════════════════════════════════════════════════════
    var oldFeatures = @json(old('features'));
    var oldBenefits = @json(old('benefits'));
    @php
        $existingFeatures = $product->features->sortBy('sort_order')->map(function($f) {
            return [
                'title' => $f->title,
                'icon'  => $f->icon,
                'items' => $f->items->sortBy('sort_order')->pluck('text')->toArray(),
            ];
        })->values();
        $existingBenefits = $product->benefits->sortBy('sort_order')->map(function($b) {
            return [
                'title' => $b->title,
                'icon'  => $b->icon,
            ];
    ...
    var featureData = oldFeatures;
    if (!featureData || Object.keys(featureData).length === 0) {
        featureData = @json($existingFeatures);
    }

    var benefitData = oldBenefits;
    if (!benefitData || Object.keys(benefitData).length === 0) {
        benefitData = @json($existingBenefits);
    }
    ...
    // Render benefits
    if (benefitData && (Array.isArray(benefitData) ? benefitData.length : Object.keys(benefitData).length)) {
        $.each(benefitData, function (idx, bData) {
            if (!bData) return;
