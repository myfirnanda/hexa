...
<div class="admin-card border rounded-xl overflow-hidden">
    <div class="p-5">
        <form method="POST" action="{{ route('manager.clients.update', $client) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <div class="text-xs admin-text-muted mt-2 flex items-center gap-2">
                    Logo saat ini: <img src="{{ image_url($client->logo) }}" class="h-7 rounded bg-white p-0.5" alt="">
                </div>
                @endif
