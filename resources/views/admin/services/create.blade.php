...
@section('topbar-title', 'Tambah Layanan')

@section('content')
<div class="mb-5">
    <a href="{{ route('manager.services.index') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-[13px] admin-btn-secondary no-underline border cursor-pointer transition-all duration-150">
...
<div class="flex items-center justify-between flex-wrap gap-4 mb-7 max-md:flex-col max-md:items-start">
    <h1 class="text-2xl font-bold admin-text">Tambah Layanan Baru</h1>
</div>
        ...
        <form method="POST" action="{{ route('manager.services.store') }}">
            ...
            <div class="grid grid-cols-2 gap-4 max-md:grid-cols-1">
                <div class="mb-5">
                    ...
                    <input type="text" class="w-full px-3.5 py-2.5 rounded-lg border admin-input font-[inherit] text-sm outline-none transition-colors duration-200 focus:border-blue-500" id="name" name="name" value="{{ old('name') }}" required placeholder="Nama layanan...">
            ...
            <div class="mb-5">
                ...
                <textarea class="w-full px-3.5 py-2.5 rounded-lg border admin-input font-[inherit] text-sm outline-none transition-colors duration-200 focus:border-blue-500 resize-y min-h-[100px]" id="description" name="description" required placeholder="Deskripsi layanan...">{{ old('description') }}</textarea>
            ...
            <div class="mb-5">
                ...
                <input type="text" class="w-full px-3.5 py-2.5 rounded-lg border admin-input font-[inherit] text-sm outline-none transition-colors duration-200 focus:border-blue-500" id="icon" name="icon" value="{{ old('icon') }}" placeholder="Nama material icon (mis: code)">
            ...
            <div class="flex gap-2.5 pt-2">
                ...
                <a href="{{ route('manager.services.index') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold text-[13px] admin-btn-secondary no-underline border cursor-pointer transition-all duration-150">Batal</a>
            </div>
        </form>
    </div>
</div>
